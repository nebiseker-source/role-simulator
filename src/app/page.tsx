...
export default function Home() {
  ...
  async function runTeamSimulation() {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : "Bilinmeyen hata";
      setTeamOutput({ steps: [], finalSynthesis: `**Hata:** ${message}`, fallbackUsed: false });
    } finally {
      setLoading(false);
    }
  }
        ...
        <section className="relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-cyan-700 via-sky-700 to-indigo-700 p-6 md:p-8">
          ...
          <div className="relative">
            ...
            <p className="mt-3 max-w-4xl text-sm text-sky-100/90 md:text-base">
              Tek rol veya ekip (BA → PO → İş Mimarı → Data Bilimci) simülasyonu çalıştır. Çıktılar görev kırılımı,
              test senaryoları, teslimat listesi ve renkli diyagramları da içerecek şekilde üretilecek.
            </p>
          </div>
        </section>
        ...
        <section className="mt-6 rounded-2xl border border-white/10 bg-white p-5 text-slate-800 shadow-2xl shadow-black/20">
          ...
          {activeResult === "single" ? (
            <div className="min-h-[72vh] max-h-[85vh] overflow-auto rounded-xl border border-slate-200 bg-slate-50 p-5">
              ...
              {singleOutput ? <MarkdownOutput content={singleOutput} /> : <p className="text-slate-500">Henüz tek rol çıktısı yok.</p>}
            </div>
          ) : null}
          ...
          {activeResult === "team" ? (
            <div className="space-y-3 min-h-[72vh] max-h-[85vh] overflow-auto">
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-5">
                ...
                <MarkdownOutput content={teamOutput?.finalSynthesis ?? "Henüz ekip çıktısı yok."} />
              </div>
          ...
          {!activeResult ? (
            <div className="min-h-[72vh] rounded-xl border border-slate-200 bg-slate-50 p-5">
              <p className="text-slate-500">Henüz çıktı yok. Yukarıdan simülasyon başlat.</p>
            </div>
          ) : null}
        </section>
      </div>
